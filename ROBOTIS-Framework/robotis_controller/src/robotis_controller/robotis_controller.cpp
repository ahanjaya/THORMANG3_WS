    ...
    else if (gazebo_mode_ == true)
    {
      ...
      for (auto& dxl_it : robot_->dxls_)
      {
        ...
        if (dxl->ctrl_module_name_ == "none")
        {
          ...
          velocity_msg.data = dxl_state->goal_velocity_;
          effort_msg.data = dxl_state->goal_torque_;

          gazebo_joint_position_pub_[joint_name].publish(joint_msg);
          gazebo_joint_velocity_pub_[joint_name].publish(velocity_msg);
          gazebo_joint_effort_pub_[joint_name].publish(effort_msg);
        }
      }
